...
class Cora(Dataset):
    ...
    def __init__(self, path, mode, num_layers,
        ...
        if transductive:
            idx = np.arange(content.shape[0]) # transductive이면 전 노드 사용
        else:
            if mode == 'train':
                idx = self.idx['train']
            elif mode == 'val':
        ...
        # 노드별 feature, label을 구한다. label의 경우 0부터 시작하는 숫자로 변환한다.
        ...
        # vertext를 구축한다. vertex별로 0부터 시작하는 id를 부여하여 변환한다.
        ...
        # coo_matrix constructor를 이용해서 sparse adjaency matrix를 구축한다.
        n, m = labels.shape[0], edges.shape[0] # num of nodes, num of edges
        u, v = edges[:, 0], edges[:, 1] # from nodes, to nodes
        adj = sp.coo_matrix((np.ones(m),(u, v)),  # (data, row/column indices)
                            shape=(n,n),
                            dtype=np.float32)
        ...
        if self_loop:
            adj += sp.eye(n)
        if normalize_adj:   # D^(-1/2) x A x D(^1/2)
    ...
    def __getitem__(self, idx):
        ...
        if self.transductive:
            ...
            if self.mode != 'train':
                idx += len(self.idx['train'])  # skip train data

        # 해당 노드를 중심으로 한 computation 그래프를 구축한다.
        # layer별 노드 목록, layer별 노드 위치 정보
        node_layers, mappings = self._form_computation_graph(idx)

        # 첫번째 layer 노드(자신)에 연결된 이웃노드들의 feature와 label을 리턴
    ...
    def collate_wrapper(self, batch):
        ...
        batch : list
            ...
            각 item은 __getitem__의 리턴값 (features, node_layers, mappings, rows, labels)
    ...
    def get_dims(self):
        ...
        :return: dimension of input feature, output features
        """
    ...
    def _form_computation_graph(self, idx):
        ...
        _list, _set = list, set
        rows = self.adj.rows    # List of List 형태의 sparse matrix
        if type(idx) is int:
        ...
        for _ in range(self.num_layers):
            prev = node_layers[-1]
            arr = [node for node in prev]  # 이전 layer에 위치한 노드들
            arr.extend([ v for node in arr for v in rows[node]])   # 이전 layer에 연결된 모든 neigbhor들을 다 추가
            arr = np.array(_list(_set(arr)), dtype=np.int64)
            node_layers.append(arr)
        node_layers.reverse()

        # 각 layer별로 node의 위치(?) 지정
