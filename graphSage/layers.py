...
class PoolAggregator(Aggregator):
    ...
    def __init__(self, input_dim, outpput_dim, device='cpu'):

        super(PoolAggregator, self).__init__(input_dim, outpput_dim, device)

        self.fc1 = nn.Linear(input_dim, outpput_dim)
        self.relu = nn.ReLU()
...
class LSTMAggregator(Aggregator):
    ...
    def _aggregate(self, features):
        ...
        features = features.unsqueeze(0)   # (batchSize=1, seq=# of neighbor, feature_dim)

        # (batch, seq_len,  num_directions * hidden_size)
        # (1, # of neighbor, 2*output_dim)
